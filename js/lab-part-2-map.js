...
let newArray = mneumonics.map(function (x) {
  return x.split(" ");
});
...
let newArr = cities.map(function (x) {
  let object = {
    name: x.name,
    country: x.country,
  };
...
// (3) Take the phonebook array and use map to return a new phonebook
//that includes a country-code that matches the rest of the phone number.
